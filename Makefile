...
DOCKER_TAG?=jaytwo_http_authentication
...
build: restore
	dotnet build ./jaytwo.Http.Authentication.sln
...
unit-test: build
	...
	rm -rf out/coverage
	cd ./test/jaytwo.Http.Authentication.Tests; \
		dotnet test \
		--results-directory ../../out/testResults \
		--logger "trx;LogFileName=jaytwo.Http.Authentication.Tests.trx"
	reportgenerator \
...
pack:
	rm -rf out/packed
	cd ./src/jaytwo.Http.Authentication; \
